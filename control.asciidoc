...
quantities occurring in real-expression see link:format#sec.format.areal[Real Expressions]. Whenever an
...
This statement creates a new global variable `variable-name` and
discards any old variable with the same name. Its value depends on all
quantities occurring in vector-expression see link:format#sec.format.anarray[Arrays] on the
...
The python script
http://amas.web.psi.ch/opal/Documentation/master/examples/gen_seed.py[gen_seed.py]
is called with the `SYSTEM` command.
This scripts writes the seed to a file
http://amas.web.psi.ch/opal/Documentation/master/examples/gen_seed.opal[gen_seed.opal]
