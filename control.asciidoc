...
`label` is an identifier see Section link:format#sec.format.label[Identifiers or Labels]. If it is non-blank, _OPAL_
...
`pattern` is an regular expression see Section link:format#sec.format.wildcard[Regular Expressions]. If it is
...
occurs in a `CALL` file see Section_<<sec.control.call>>, returns to the calling file.
...
SEED::
  ...
  see Section link:format#sec.format.adefer[Deferred Expressions and Random Values].
PSDUMPFREQ::
...
RNGTYPE::
  The name see Section link:format#sec.format.astring[String Attributes] of a random number generator can be
...
quantities occurring in real-expression see Section link:format#sec.format.areal[Real Expressions]. Whenever an
...
by using the `EVAL` function see Section <<sec.control.eval>>.
...
This statement creates a new global variable `variable-name` and
discards any old variable with the same name. Its value depends on all
quantities occurring in vector-expression see Section link:format#sec.format.anarray[Arrays] on the
...
This statement creates a new global variable `variable-name` and
discards any old variable with the same name. Its value depends on all
quantities occurring in logical-expression see Section link:format#sec.format.alogical[Logical Expressions].
...
see Section link:format#sec.format.anarray[Arrays]. Whenever an operand changes in
...
`message` is a string value see Section link:format#sec.format.astring[String Attributes]. It is immediately
...
The string see Section link:format#sec.format.astring[String Attributes] `string` must be a valid operating
system command.

[[sec.control.system-command-under-unix]]
...
`page-header` is a string value see Section link:format#sec.format.astring[String Attributes]. It defines the
...
`file-name` is a string see Section link:format#sec.format.astring[String Attributes]. The statement causes the
...
`file-name` is a string see Section link:format#sec.format.astring[String Attributes]. The command causes all
