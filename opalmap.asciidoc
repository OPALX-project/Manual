...
The map creation is done by applying the Lie Operator on the element Hamiltonian and calculated in the Truncated Power Series Algebra (TPSA)**[ref Bertz]**. The TPSA is a Differential Algebra (DA), which uses the Taylor expansion as the equivalent function. In OPAL-map the TPSA gets provided from the own OPAL DA-package. 
...
[bibliography]
.References
- [[[TPSA]]] Eric Steven Raymond. 'The Art of UNIX
