...
|`FSTYPE` |Specify the type of field solver: `FFT`, `FFTPERIODIC`, `MG` and `NONE`. AMR solvers:
`FMG`, `ML` and `AMR_MG`
...
|`AMR_TAGGING` |Mesh-refinement strategy [`CHARGE_DENSITY,POTENTIAL,EFIELD,
...
|`AMR_SCALING` | Refinement threshold in case of `AMR_TAGGING=POTENTIAL` or
`AMR_TAGGING=POTENTIAL

|`AMR_DOMAIN_RATIO` | Box ratio of the AMR computation domain. It is an array of size 3.
The entries define the ratio in (x, y, z) direction. For example, [1, 0.75, 0.75] yields
...
commands to be used in the Fieldsolver command. In order to run a proper AMR
simulation one requires following ingredients:

* Upper bound of refinement, specified by `AMR_MAXLEVEL`
* Maximum allowable grid size for the base level, specified by `AMR_MAXGRIDX`,
`AMR_MAXGRIDY` and `AMR_MAXGRIDZ`. The maximum grid size of the next higher level is
given by the division with the grid cell refinement ratio. The same is true for
the blocking factors.
* Mesh refinement strategy, specified by `AMR_TAGGING`. Depending on the
tagging scheme, further keywords can be used.

See also <<tab_FIELDSOLVER_Commands>> and <<tab_Mesh_refinement_strategies>>.
...
* Tpetra <<bib.tpetra>>
* Ifpack2 <<bib.ifpack2>>
* Amesos2 and Belos <<bib.amesos2-belos>>
* MueLu <<bib.muelu>>
...
[`NONE,ILUT,CHEBYSHEV,RILUK,SA,JACOBI,BLOCK_JACOBI,GS,BLOCK_GS`]
...
[`BICGSTAB,MINRES,PCPG,CG,GMRES,STOCHASTIC_CG,RECYCLING_CG,RECYCLING_GMRES,
KLU2,SUPERLU,UMFPACK,PARDISO_MKL,MUMPS,LAPACK,SA`]. Please see
