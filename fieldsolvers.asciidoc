...
commands to be used in the Fieldsolver command
(cf. <<tab_AMR_extensions_fieldsolver_command>>). The user specifies the
maximum grid size of the base level. The size of the next higher level is
given by the division with the grid cell refinement ratio. The same is true for
the blocking factors.

Depending on the tagging scheme, further keywords can be used.
