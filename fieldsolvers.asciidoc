...
command as described in Chapter link:track#chp.track[Tracking]. By default,
...
takes in to account collisions between particles in an electrostatic 
one-one PIC simulation, i.e. every simulation particle is a
real particle, in an efficient manner compared to PIC with excessive mesh refinement or
a direct N-body summation. The main idea behind this appraoch is a splitting of the 
...
At the moment, `P3M` solver is only available in _OPAL-T_ and that too not during emission. We can 
...
calculated using direct summation. Deafult value is 0 which corresponds to no particle-particle part.
...
Default value is latexmath:[1e8].   
...
(see link:elements#sec.elements.cyclotron[Cyclotron]) the number of energy
