...
The `DUMPEMFIELDS` statement causes _opal-cycl_ to write out electromagnetic field data on a 4D grid. Grids in a Cartesian coordinate system $(x,y,z,t)$ and a cylindrical coordinate system about the z-axis in $(r,\phi$,z,t)$ are supported. 
