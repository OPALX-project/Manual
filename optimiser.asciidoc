...
image::figures/optimiser/design_objective_space.png[scaledwidth=10cm]
...
image::figures/optimiser/pareto-defs.png[scaledwidth=18cm]
...
| DUMP_FREQ                 | Dump generation data format with frequency.
...
| MUTATION                  | Mutation of an individual ("OneBit" (single gene), "IndependentBit" (default, multiple genes))
...
[#tab_objectivecommandattributes]
...
[#tab_constraintcommandattributes]
...
| sddsVariableAt(<var>, <spos>, <sdds_file>)
| A simple expression to get SDDS value near a specific spos for a variable.

| statVariableAt(<var>, <spos>)
...
OPTIMIZE, INPUT="tmpl/05-DL_QN3.tmpl", OBJECTIVES = {drmsx, drmsy, goalfun},
          ...
          NUM_IND_GEN=3, MUTATION_PROBABILITY=0.43, SIMBIN_CROSSOVER_NU=2.0,
