...
FMAPFN::
  ...
  see <<sec.elements.elements.benddefaultfieldmapopalt>>. The field map is used to
...
image::figures/Elements/sbend.png[width=10cm]
...
two types of bends see <<sec.elements.RBend>> and <<sec.elements.SBend>>, and an `SBEND` has an
...
.Plot of the entrance fringe field of a dipole magnet along the mid-plane, perpendicular to its entrance face. The field is normalized to 1.0. In this case, the fringe field is described by an Enge function see Equation [enge_func] with the parameters from the default `1DProfile1` field map described in <<sec.elements.benddefaultfieldmapopalt>>. The exit fringe field of this magnet is the mirror image.
image::figures/Elements/Enge-func-plot.png[width=10cm]
...
of Gauss, where the `Dipole` example (<<fig_sbend3d1>>) uses meter and
...
image::figures/Elements/sbend3d.png[width=18cm]
...
in the RFCAVITY element see <<cavity>> and make a momentum kick
...
image::./figures/cyclotron/Cavity.png[width=10cm]
...
image::./figures/traveling-wave-structure/FINSB-RAC-field.png[width=10cm]
...
image::figures/Elements/pepperpot.png[width=10cm]
...
.Collimator
image::figures/Elements/collimator.png[width=10cm]
...
.Septum
image::figures/Elements/septum.png[width=10cm]
...
.Probe
image::figures/Elements/probe.png[width=10cm]
