...
Example of an straight interacion point element at 42m and with 1m in lenght:

----
ip1: IP, ELEMEDGE = 44.0, COLWINLEN = 0.1;
----

As soon as the first particle enters the IP region a second bunch is created at ELEMEDGE+COLWINLEN with p = (px,py,-pz).

COLWINLEN::
  Defines the lenght [m] over which the coulomb interaction of the collisons are calculeted.
