...
label::
  ...
  identifier see Section link:format#sec.format.label[Identifiers or Labels].
keyword::
  Is a keyword see Section link:format#sec.format.label[Identifiers or Labels], it is an element type keyword (in
...
attribute-name::
  ...
  see Section link:format#sec.format.label[Identifiers or Labels].
attribute-value::
  gives it a value see Section link:format#sec.format.attribute[Command Attribute Types] (in the example `1.8` and
  `0.015832`).
...
TYPE::
  A string value see Section link:format#sec.format.astring[String Attributes]. It specifies an "engineering
  type" and can be used for element selection.
APERTURE::
  A string value see Section link:format#sec.format.astring[String Attributes] which describes the element
...
in _OPAL-t_, see Section_<<sec.elements.RBend>>), `SBEND` (valid in _OPAL-t_, see
Section <<sec.elements.SBend>>), `RBEND3D`, (valid in _OPAL-t_, see Section <<sec.elements.RBend3D>>)
and `SBEND3D` (valid in _OPAL-cycl_, see Section <<sec.elements.SBend3D>>).
...
K1::
  ...
  where latexmath:[R] is the bend radius as defined in Figure <<fig_rbend,1>>.
  Not supported in `DOPAL-t` any more. Superimpose a `Quadrupole`
  instead.
E1::
  Entrance edge angle (radians). Figure <<fig_rbend,1>> shows the definition of
...
DESIGNENERGY::
  ...
  approximately the path shown in Figure <<fig_rbend,1>>.
FMAPFN::
  ...
  see Section_<<sec.elements.benddefaultfieldmapopalt>>. The field map is used to
  describe the fringe fields of the magnet see Section link:fieldmaps#sec.fieldmaps.1DProfile1[`1DProfile1`].

[[sec.elements.RBend3D]]
==== RBend3D (_OPAL-t_)

An `RBEND3D3D` is a rectangular bending magnet. The key property of an
`RBEND3D` is that it has parallel pole faces. Figure <<fig_rbend,1>> shows an
...
FMAPFN::
  ...
  link:fieldmaps#sec.fieldmaps.1DProfile1[`1DProfile1`] can be used. The default option
  for this attribute is `FMAPN` = `1DPROFILE1-DEFAULT`
  see Section <<sec.elements.benddefaultfieldmapopalt>>. The field map is used to
  describe the fringe fields of the magnet link:fieldmaps#sec.fieldmaps.1DProfile1[`1DProfile1`].

[#fig_sbend]
Figure <<fig_sbend>> illustrates a general sector bend(`SBEND`) with a positive bend angle latexmath:[\alpha]. In this example the entrance and exit edge angles latexmath:[E_{1}] and latexmath:[E_{2}] have positive values. For a positively charge particle, the magnetic field is directed out of the page.
...
entrance and exit edge angles. Figure <<fig_sbend,2>> shows an `SBEND` with a
positive bend angle, a positive entrance edge angle, and a positive exit
edge angle.

L::
  Chord length of the bend reference arc in meters (see Figure <<fig_sbend,2>>),
...
K1::
  Field gradient index of the magnet,
  latexmath:[K_1=-\frac{R}{B_{y}}\frac{\partial B_y}{\partial x}],
  where latexmath:[R] is the bend radius as defined in Figure <<fig_sbend,2>>.
  Not supported in `DOPAL-t` any more. Superimpose a `Quadrupole`
  instead.
E1::
  Entrance edge angle (rad). Figure <<fig_sbend,2>> shows the definition of a
  positive entrance edge angle.
E2::
  Exit edge angle (rad). Figure <<fig_sbend,2>> shows the definition of a
  positive exit edge angle.
DESIGNENERGY::
  Energy of the bend reference particle (MeV). The reference particle
  travels approximately the path shown in Figure <<fig_sbend,2>>.
FMAPFN::
  Name of the field map for the magnet. Currently maps of type
  link:fieldmaps#sec.fieldmaps.1DProfile1[`1DProfile1`] can be used. The default option
  for this attribute is `FMAPN` = `1DPROFILE1-DEFAULT`
...
two types of bends see Sections <<sec.elements.RBend>> and <<sec.elements.SBend>>, and an `SBEND` has an
additional attribute `E2` that has no effect on an `RBEND`, see
Section <<sec.elements.SBend>>. Therefore, in this section, we will give several
...
Figures <<fig_rbend,1>> and <<fig_sbend,2>>. Bends in other directions within the x/y plane are
...
trajectory, as shown in Figures <<fig_rbend,1>> and <<fig_sbend,2>>.
5.  If the default field map, `1DPROFILE-DEFAULT`
see Section <<sec.elements.benddefaultfieldmapopalt>>, is used, the fringe fields will
...
the default field map see Section <<sec.elements.benddefaultfieldmapopalt>> for
describing the magnet fringe fields see Section link:fieldmaps#sec.fieldmaps.1DProfile1[`1DProfile1`]:
...
Figure <<fig_rbend,1>>). It has a design energy of 10 MeV, a length of 0.5 m, a
...
see Section <<sec.elements.benddefaultfieldmapopalt>> which describes the magnet fringe
fields see Section link:fieldmaps#sec.fieldmaps.1DProfile1[`1DProfile1`]. When _OPAL_ is run, you will get the
...
trajectory like that described in Figure <<fig_rbend,1>>. From the value of
...
see Section link:fieldmaps#sec.fieldmaps.1DProfile1[`1DProfile1`]. So, once the hard edge bend/reference
...
direction (as in Figure <<fig_rbend,1>>) and then use the `ROTATION` attribute
...
attribute according to Sections <<sec.elements.RBend>> and <<sec.elements.SBend>>, and by adding the
...
see Section link:fieldmaps#sec.fieldmaps.1DProfile1[`1DProfile1`]:
...
the entrance and exit points of the magnet see Section link:fieldmaps#sec.fieldmaps.1DProfile1[`1DProfile1`].
...
We have moved the Enge function origins see Section link:fieldmaps#sec.fieldmaps.1DProfile1[`1DProfile1`] outward
from the entrance and exit faces of the magnet see Section link:fieldmaps#sec.fieldmaps.1DProfile1[`1DProfile1`]
...
.Figure 3: Plot of the entrance fringe field of a dipole magnet along the mid-plane, perpendicular to its entrance face. The field is normalized to 1.0. In this case, the fringe field is described by an Enge function see Equation [enge_func] with the parameters from the default `1DProfile1` field map described in Section <<sec.elements.benddefaultfieldmapopalt>>. The exit fringe field of this magnet is the mirror image.
...
This can be seen clearly in Figure link:fieldmaps#fig_rbend_field_profile[3 of Chapter Fieldmaps].

The purpose of the `1DProfile1` field map see Section link:fieldmaps#sec.fieldmaps.1DProfile1[`1DProfile1`]
...
field is shown in Figure <<fig_rbend_enge_fringe,3>>.

Let us assume we have a correctly defined positive `RBEND` or `SBEND`
element as illustrated in Figures <<fig_rbend,1>> and <<fig_sbend,2>>. (As already stated, any
...
`1DProfile1` (see Section link:fieldmaps#sec.fieldmaps.1DProfile1[`1DProfile1`] for an explanation of the field
map format) map:
...
magnet (see Sections <<sec.elements.RBend>> and <<sec.elements.SBend>>).
...
of Gauss, where the `Dipole` example (Figure <<fig_sbend3d1,4>>) uses meter and
...
KN::
  A real vector see Section link:format#sec.format.anarray[Arrays], containing the normal multipole
  coefficients, latexmath:[K_n=\frac{\partial{^n} B_y}{\partial x^n}].
  (default is 0 latexmath:[\mathrm{Tm^{-n}}]). A component is positive, if
  latexmath:[B_y] is positive on the positive latexmath:[x]-axis.
KS::
  A real vector see Section link:format#sec.format.anarray[Arrays], containing the skew multipole
  coefficients,
...
Section <<sec.elements.quadrupole>>.
...
=== General Multipole (will replace Section <<sec.elements.multipole>> when implemented)
...
TP::
  A real vector see Section link:format#sec.format.anarray[Arrays], containing the multipole
...
in the RFCAVITY element see Section <<sec.elements.cavity>> and make a momentum kick
...
FAST::
  ...
  map is constructed from the 1D on-axis field, see Section link:fieldmaps#sec.fieldmaps.type_and_format[Fieldmaps Types and Format].
...
_<inputfilename>.h5_ (or ASCII if link:control#sec.control.option[`ASCIIDUMP`] is true).
...
* [[[bib.tait-bryan_elements,bib.tait-bryan]]] https://en.wikipedia.org/wiki/Euler_angles#Tait.E2.80.93Bryan_angles[_Tait-bryan angles_].
* [[[bib.enge_elements]]] J. E. Spencer and H. A. Enge, _Split-pole magnetic spectrograph for precision nuclear spectroscopy_, Nucl. Instrum. Methods 49, 181–193 (1967).
