...
keyword::
  Is a keyword (see link:format#sec.format.label[Identifiers or Labels]),
...
attribute-value::
  gives it a value (see link:format#sec.format.attribute[Command Attribute Types])
...
TYPE::
  A string value (see link:format#sec.format.astring[String Attributes]).
  It specifies an "engineering type" and can be used for element selection.
APERTURE::
  A string value (see link:format#sec.format.astring[String Attributes]) which
...
fields see link:fieldmaps#sec.fieldmaps.1DProfile1[`1DProfile1`]. When _OPAL_
...
(see link:fieldmaps#sec.fieldmaps.1DProfile1[`1DProfile1`]). So, once the
...
We have moved the Enge function origins (see link:fieldmaps#sec.fieldmaps.1DProfile1[`1DProfile1`])
...
`1DProfile1` (see link:fieldmaps#sec.fieldmaps.1DProfile1[`1DProfile1`] for an
...
KN::
  A real vector (see link:format#sec.format.anarray[Arrays]), containing the
...
--
This model includes fringe fields. The detailed description can be found at:
...
----
ip1: IP, ELEMEDGE = 44.0, COLWINLEN = 0.1, VISUALIZE = TRUE;
----
...
Chapter link:collisions#chp.collisions[Physics Models Used in Modelling Collisions].
...
IP1: IP, L = 0.1, ELEMEDGE = 0.1, COLWINLEN = 0.05, VISUALIZE = TRUE, COPY = TRUE;
...
TRACK, LINE = myLine, SOURCES = mySource, BEAM = BEAM1, MAXSTEPS = {3,40}, DT = {100e-12,10e-12};
 RUN, METHOD = "PARALLEL", BEAM = BEAM1, FIELDSOLVER = FS1;
ENDTRACK;
QUIT;
----
...
...
DR1: DRIFT, L = 0.1, ELEMEDGE = 0.0;
IP1: IP, L = 0.1, ELEMEDGE = 0.1, COLWINLEN=0.05, VISUALIZE=TRUE COPY=FALSE;
DR2: DRIFT, L = 0.1, ELEMEDGE = 0.1;
...
TRACK, LINE = myLine, SOURCES = mySources, BEAM = BEAM1, MAXSTEPS = {3,40}, DT = {100e-12,10e-12};
 RUN, METHOD = "PARALLEL", BEAM = BEAM1, FIELDSOLVER = FS1;
ENDTRACK;
QUIT;
----
