...
read from external h5part file and the following extra arguments need to specified:

RFMAPFN::
  The file name(s) for the electric field map(s) in h5part binary format.
RFPHI::
...
TYPE::
  ...
  The general PSI-BFIELD and PSI-PHASE descriptions are based on a rational function with polynomials in the nominator and the denominator.
...
BMAX::
  Maximal B field of the trim coils [T] or maximal phase shift (for PSI-PHASE)
COEFNUM::
  Coefficients of the numerator, first coefficient is zeroth order (not for PSI-BFIELD-MIRRORED)
COEFDENOM::
...
image::./figures/traveling-wave-structure/FINSB-RAC-field.png[scaledwidth=10cm]
...
image::figures/Elements/intersection.png[scaledwidth=10cm]
...
image::figures/Elements/pepperpot.png[scaledwidth=10cm]
...
.Collimator
image::figures/Elements/collimator.png[scaledwidth=10cm]
...
.Septum
image::figures/Elements/septum.png[scaledwidth=10cm]
...
.Probe
image::figures/Elements/probe.png[scaledwidth=10cm]
...
The particles recorded in the PROBE are also recorded in the histogram ".hist" and peak ".peaks" file.
...
stripper. The charge and mass are changed. It has the same geometry as
