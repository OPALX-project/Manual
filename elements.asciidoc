...
Repeats the holes that are defined by the command translating each copy successively by shiftx in x-direction and shifty in y-direction.
repeat(command, N, angle)::
Repeats the holes that are defined by the command rotating each copy successively.
...
YEND::
  The y coordinate of the end point. [mm]
WIDTH::
  The width of the stripper, NOT used yet.
OPCHARGE::
