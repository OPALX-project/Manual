...
link:beam-command#chp.beam-command[`BEAM`] must be selected. In _OPALX_, a
single beam can be selected with `BEAM`, or multiple beams can be selected with
`BEAMS`. The time step (`DT`) and the maximal steps to track (`MAXSTEPS`) or
`ZSTOP` should be set. This command causes _OPAL_ to enter "tracking mode", in
which it accepts only the track commands (see <<tab_TRACK_Commands>>). In order
to perform several tracks, specify arrays of parameter in `DT`, `MAXSTEPS` and
...
[.feature-opal]
METHOD::
  The name (a string, see link:format#sec.format.astring[String Attributes])
...
BEAM::
  In _OPAL_, the particle beam (see Chapter link:beam-command#chp.beam-command[Beam Command]) to be used is specified here.
+
...
MBMODE::
  ...
  namely, `AUTO` and `FORCE`. See link:opalcycl#sec.opalcycl.MultiBunch[Multi-bunch Mode]
