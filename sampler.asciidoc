...
The two sampling methods differ in the number of samples since with `SEQUENCE = TRUE` every combination of individual sampling is computed. Thus the total number is latexmath:[N = N_1 \times N_2 \times \ldots \times N_n] where latexmath:[n] is the number of `DVARS`. With `SEQUENCE = FALSE` the number of sampling points is latexmath:[N = \min(N_1, N_2,\dots, N_n)]. 

latexmath:[\beta_y\gamma] 
