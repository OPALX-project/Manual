...
link:elements#sec.elements.RBend[RBend (_OPAL-t_)] and link:elements#sec.elements.SBend[SBend (_OPAL-t_)].
...
1DMagnetoStatic::
  1D magnetostatic field map. See Section <<sec.fieldmaps.1DMagnetoStatic>>.
AstraMagnetoStatic::
  ...
  See Section <<sec.fieldmaps.AstraMagnetostatic>>.
1DDynamic::
  1D dynamic electromagnetic field map. See Section <<sec.fieldmaps.1DDynamic>>.
AstraDynamic::
  ...
  small changes. See Section <<sec.fieldmaps.AstraDynamic>>.
1DProfile1::
  ...
  see Sections link:elements#sec.elements.RBend[RBend (_OPAL-t_)] and link:elements#sec.elements.SBend[SBend (_OPAL-t_)]. See Section <<sec.fieldmaps.1DProfile1>>.
2DElectroStatic::
  2D electrostatic field map. 2D field maps are described by the
  electromagnetic field in one half-plane. See
  Section <<sec.fieldmaps.2DElectroStatic>>.
2DMagnetoStatic::
  ...
  Section <<sec.fieldmaps.2DMagnetoStatic>>.
2DDynamic::
  ...
  Section <<sec.fieldmaps.2DDynamic>>.
3DElectroStatic::
  3D electrostatic field map. +
  *_Not implemented yet_*.
3DMagnetoStatic::
  3D magnetostatic field map, see Section <<sec.fieldmaps.3DMagnetoStatic>>.
3DMagnetoStatic_Extended::
  2D magnetostatic field map of field on mid-plane that _OPAL_ extends
  to 3D.
3DDynamic::
  3D dynamic electromagnetic field map. See Section <<sec.fieldmaps.3DDynamic>>.
...
be used! Figure <<fig_fieldnoiseexample,2>> shows three pictures of the
longitudinal phase space after three gun simulations using different
types of field maps. In the first picture we
used a 1DDynamic field map see Section <<sec.fieldmaps.1DDynamic>> resulting in a smooth
...
Table <<tab_1DMagnetoStatic>>. The first three lines form the file header and
...
1DMagnetoStatic see Section <<sec.fieldmaps.1DMagnetoStatic>> field maps, _OPAL-t_
...
A `1DDynamic` field map has the general form shown in Table <<tab_1DDynamic>>.
...
shown in Figure <<fig_rbend_field_profile>>. In the top part of the figure we
...
`SBEND` elements see Sections link:elements#sec.elements.RBend[RBend (_OPAL-t_)], link:elements#sec.elements.SBend[SBend (_OPAL-t_)] and link:elements#sec.elements.opaltrbendsbendfields[Bend Fields from 1D Field Maps (_OPAL-t_)].

[#fig_rbend_field_profile]
...
Table <<tab_1DProfile1>>. The first three lines form the file header and tell
_OPAL-t_ how the field map data is being presented:
...
Line 2::
  ...
  see Sections <<sec.fieldmaps.1DProfile1Type1>> and <<sec.fieldmaps.1DProfile1Type2>>. The fourth value on
...
Line 3::
  ...
  see Sections_<<sec.fieldmaps.1DProfile1Type1>> and <<sec.fieldmaps.1DProfile1Type2>>. The fourth value on
...
This will be explained in Sections <<sec.fieldmaps.1DProfile1Type1>> and
...
Figures <<fig_rbendengetype1>> and <<fig_sbendengetype1>> illustrate the fringe field
regions for an `RBEND` and an `SBEND` element. Referring to the general
field map file shown in Table <<tab_1DProfile1>>, the values on lines 2 and 3
are given by:
...
points see Figures <<fig_rbendengetype1>> and <<fig_sbendengetype1>>.
...
Table <<tab_1DProfile1>> are defined. Now
...
see Section <<sec.fieldmaps.fieldorientation>>. _OPAL-t_ normalizes the field so that
...
see Section <<sec.fieldmaps.fieldorientation>> and can be one of two formats:
...
Table <<tab_2DMagnetoStatic>>. The first three lines form the file header and
tell _OPAL-t_ how the field map data is being presented:
...
on the field orientation see Section <<sec.fieldmaps.fieldorientation>>. _OPAL-t_
...
A `2DDynamic` field map has the general form shown in Table <<tab_2DDynamic>>.
The first four lines form the file header and tell _OPAL-t_ how the
field map data is being presented:

Line 1::
  This tells _OPAL-t_ what type of field file it is (`2DDynamic`) and
  the field orientation see Section <<sec.fieldmaps.fieldorientation>>.
Line 2::
  This gives the extent of the field map and how many grid spacings
  there are in the fastest changing index direction
  see Section <<sec.fieldmaps.fieldorientation>>.
...
Table <<tab_3DMagnetoStatic>>. The first five lines form the file header and
tell _OPAL-t_ how the field map data is being presented:
...
Table <<tab_3DMagnetoStatic_Extended>>. The first four lines form the file
...
order of these depend on the field orientation
see Section <<sec.fieldmaps.fieldorientation>> and can currently only be the format
shown in Table <<tab_3DMagnetoStatic_Extended>>.
...
A `3DDynamic` field map has the general form shown in Table <<tab_3DDynamic>>.
