...
link:opalt#sec.opalt.variablesopalt[_OPAL-t_ variables] and link:opalcycl#sec.opalcycl.variables[_OPAL-cycl_ variables].
When defining a
...
defined by link:opalt#sec.opalt.variablesopalt[_OPAL-t_ variables], is given in seconds instead of
...
see link:fieldsolvers#sec.fieldsolvers.FSENBINS[Energy Bins].
...
|`ID1` |0.0 |<<sec.distribution.unitsdistattributes>> |Tracer particle which is
written also into _data/track_orbit.dat_

|`ID2` |0.0 |<<sec.distribution.unitsdistattributes>> |Tracer particle which is
written also into _data/track_orbit.dat_
|=======================================================================
...
<<tab_fromfileinjfileformat>>. The particle coordinates are defined in link:opalt#sec.opalt.variablesopalt[_OPAL-t_ variables] and link:opalcycl#sec.opalcycl.variables[_OPAL-cycl_ variables]. The _emitted_ file format is defined in
<<tab_fromfileemitfileformat>>. The particle coordinates are defined in link:opalt#sec.opalt.variablesopalt[_OPAL-t_ variables] except that latexmath:[z], in meters, is replaced by latexmath:[t] in seconds.
[cols="<,<,<,<,<,<",]
...
command in Chapter link:beam-command#chp.beam-command[Beam Command] must be equal.
...
image::./figures/distribution/flattop.png[scaledwidth=12cm]
...
image::./figures/distribution/BinomialProperties.png[scaledwidth=18cm]
...
[[tab_distattrgaussmatched,{counter:tab-cnt}]]
[cols="<2,^1,^1,<4",options="header",]
|=======================================================================
|Attribute Name |Default Value |Units |Description
...
[[tab_distattrmultigauss,{counter:tab-cnt}]]
[cols="<2,^1,^1,<4",options="header",]
|=======================================================================
|Attribute Name |Default Value |Units |Description
...
An example (fig. <<fig_multiGaussInjected>>) of a `MULTIGAUSS` injected bunch could be:
...
see Chapter link:track#chp.track[Tracking]. Assume we have defined several distributions:
...
command see Chapter link:beam-command#chp.beam-command[Beam Command]. The number of particles in each distribution
in a distribution list is determined by this number and the `WEIGHT`
attribute of each distribution (<<tab_distattruniversal>>). If all
...
[[bib.JohoDist,*[{counter:bib-cnt}]*]] <<bib.JohoDist>> W. Joho, Representation of beam ellipses for transport calculations, PSI Internal Report TM-11-14, https://intranet.psi.ch/pub/AUTHOR_WWW/ABE/TalksDE/TM-11-14.pdf.

[[bib.laserPulseShaping,*[{counter:bib-cnt}]*]] <<bib.laserPulseShaping>> J. G. Power and C. Jing, Temporal Laser Pulse Shaping for RF Photocathode Guns: The Cheap and Easy way using UV Birefringent Crystals, AIP Conference Proceedings 1086, 689 (2009); https://doi.org/10.1063/1.3080991

[[bib.flo97,*[{counter:bib-cnt}]*]] <<bib.flo97>> K. Flöttmann, _Note on the thermal emittance of electrons emitted by Cesium Telluride photo cathodes_, tech. rep. TESLA FEL-Report (DESY, Jan. 1997).

[[bib.clen2000,*[{counter:bib-cnt}]*]] <<bib.clen2000>> J. E. Clendenin et al., _Reduction of thermal emittance of rf guns_, Nucl. Instrum. Methods Phys. Res. Sect. A 455, 198–201 (2000).

[[bib.dowe2009,*[{counter:bib-cnt}]*]] <<bib.dowe2009>> D. H. Dowell and J. F. Schmerge, _Quantum efficiency and thermal emittance of metal photocathodes_, Phys. Rev. ST Accel. Beams 12, 074201 (2009).
