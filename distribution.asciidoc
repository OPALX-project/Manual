...
in <<disttypes>>. The attributes that follow the distribution type
...
<<distattrfromfile>> we list the single attribute specific to this
...
<<fromfileinjfileformat>>. The particle coordinates are defined in link:opalt#sec.opalt.variablesopalt[_OPAL-t_ variables] and link:opalcycl#sec.opalcycl.variables[_OPAL-cycl_ variables]. The _emitted_ file format is defined in
<<fromfileemitfileformat>>. The particle coordinates are defined in link:opalt#sec.opalt.variablesopalt[_OPAL-t_ variables] except that latexmath:[z], in meters, is replaced by latexmath:[t] in seconds.
[cols="<,<,<,<,<,<",]
...
In <<distattrgauss>> we list the basic attributes available for a
...
image::./figures/distribution/flattop.png[pdfwidth=12cm]
...
have the distribution attributes listed in <<distattremittedgauss>>.
...
<<distattremittedgauss>> do not define RMS quantities, but instead
...
see <<distattrgauss>>, and is given by:
...
from <<distattremittedgauss>>. `FTOSCPERIODS` defines how many
...
<<distattrflattopinj>> and <<distattruniversal>>. At the moment, we cannot
...
defined in in <<flattop>>.

|`TPULSEFWHM` |0.0 |s |Flat top time. See <<flattop>>.

|`TRISE` |0.0 |s |Rise time. See <<flattop>>. If defined will
override `SIGMAT`.

|`TFALL` |0.0 |s |Fall time. See <<flattop>>. If defined will
override `SIGMAT`.

|`FTOSCAMPLITUDE` |0 |None |Sinusoidal oscillations can imposed on the
flat top in <<flattop>>. This defines the amplitude of those
oscillations in percent of the average flat top amplitude.
...
<<distattrflattopemit>> and <<distattruniversal>>. The `FLATTOP`
...
rise and fall time as defined in <<flattop>>.
...
<<distattrflattopemit>>. Then, _OPAL-t_ will use the laser image as
...
<<binomdist>> and shown in <<fig_binomialproperties>>.
...
[#fig_binomialproperties]
.Properties of binomial Phase Space Distributions (taken from  <<bib.JohoDist>>).
image::./figures/distribution/BinomialProperties.png[pdfwidth=18cm]

The attributes of a `BINOMIAL` distribution are defined in <<distattrbinomial>>.
...
The attributes of a `MATCHEDGAUSS` distribution are defined in <<distattrmatchedgauss>>.
...
<<distattremitmodelnoneastra>>. The `NONE` emission model is very
...
option as listed in <<distattremitmodelnoneastra>>. However, in this
...
attribute of each distribution (<<distattruniversal>>). If all
