...
simulation. Like other _OPAL_ commands (see Chapter link:format#chp.format[Command Format]),
...
(see Chapter link:beam-command#chp.beam-command[Beam Command]) must be equal.
...
(see Section link:beam-command#sec.beam-command.beam-energy[Beam Energy]).
...
link:opalcycl#sec.opalcycl.localframe[local reference frame].
...
(see Chapter link:track#chp.track[Tracking]). Assume we have defined
...
command see Chapter link:beam-command#chp.beam-command[Beam Command].
