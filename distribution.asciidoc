...
|<<sec.distribution.matchedgausstype,`MATCHEDGAUSS`>> | Initial distribution generated using matched Gaussian distribution(s).
...
Sections link:opalt#sec.opalt.variablesopalt[_OPAL-t_ variables] and link:opalcycl#sec.opalcycl.variables[_OPAL-cycl_ variables].
When defining a
...
latexmath:[\beta_{z} \gamma], as in Section link:opalt#sec.opalt.variablesopalt[_OPAL-t_ variables].
...
defined by Section link:opalt#sec.opalt.variablesopalt[_OPAL-t_ variables], is given in seconds instead of
...
defined in
Sections <<sec.distribution.universaldistattributes, Universal Attributes>>, <<sec.distribution.injecteddistattributes, Injected Distribution Attributes>> and <<sec.distribution.emitteddistattributes, Emitted Distribution Attributes>>.
...
list see Section <<sec.distribution.distlist>>.
...
see Section link:fieldsolvers#sec.fieldsolvers.FSENBINS[Energy Bins].
...
|`OFFSETPX` |0.0 |Section <<sec.distribution.unitsdistattributes>> |Distribution is shifted
in latexmath:[p_{x}] by this amount after the distribution is
generated (or read in). Same as the average latexmath:[p_{x}] value,
latexmath:[\bar{p}_{x}].

|`OFFSETPY` |0.0 |Section <<sec.distribution.unitsdistattributes>> |Distribution is shifted
in latexmath:[p_{y}] by this amount after the distribution is
generated (or read in). Same as the average latexmath:[p_{y}] value,
latexmath:[\bar{p}_{y}].

|`OFFSETPZ` |0.0 |Section <<sec.distribution.unitsdistattributes>> |Distribution is shifted
in latexmath:[p_{z}] by this amount after the distribution is
generated (or read in). Same as the average latexmath:[p_{z}] value,
latexmath:[\bar{p}_{z}].

|`ID1` |0.0 |Section <<sec.distribution.unitsdistattributes>> |Tracer particle which is
written also into _data/track_orbit.dat_

|`ID2` |0.0 |Section <<sec.distribution.unitsdistattributes>> |Tracer particle which is
written also into _data/track_orbit.dat_
|=======================================================================
...
particles from cathode see Section <<sec.distribution.emissionmodel>>.
|=======================================================================
...
Table <<fromfileinjfileformat,8>>. The particle coordinates are defined in Sections link:opalt#sec.opalt.variablesopalt[_OPAL-t_ variables] and link:opalcycl#sec.opalcycl.variables[_OPAL-cycl_ variables]. The _emitted_ file format is defined in
Table <<fromfileemitfileformat,9>>. The particle coordinates are defined in Section link:opalt#sec.opalt.variablesopalt[_OPAL-t_ variables] except that latexmath:[z], in meters, is replaced by latexmath:[t] in seconds.
[cols="<,<,<,<,<,<",]
...
`EMISSIONMODEL`, which is described in Section <<sec.distribution.emissionmodel>>. A choice
...
|`SIGMAPX` |0.0 |Section <<sec.distribution.unitsdistattributes>> |Parameter
latexmath:[\sigma_{px}] for defining distribution.

|`SIGMAPY` |0.0 |Section <<sec.distribution.unitsdistattributes>> |Parameter
latexmath:[\sigma_{py}] for defining distribution.

|`SIGMAPZ` |0.0 |Section <<sec.distribution.unitsdistattributes>> |Parameter
...
|`CORRX` |0.0 |Section <<sec.distribution.unitsdistattributes>> |latexmath:[x],
latexmath:[p_x] correlation. (latexmath:[R_{12}] in transport
notation.)

|`CORRY` |0.0 |Section <<sec.distribution.unitsdistattributes>> |latexmath:[y],
latexmath:[p_y] correlation. (latexmath:[R_{34}] in transport
notation.)

|`CORRZ` |0.0 |Section <<sec.distribution.unitsdistattributes>> |latexmath:[z],
latexmath:[p_z] correlation. (latexmath:[R_{56}] in transport
notation.)

|`CORRT` |0.0 |Section <<sec.distribution.unitsdistattributes>> |
same as and overwritten by `CORRZ`.

|`R51` |0.0 |Section <<sec.distribution.unitsdistattributes>> |latexmath:[x],
latexmath:[z] correlation. (latexmath:[R_{51}] in transport
notation.)

|`R52` |0.0 |Section <<sec.distribution.unitsdistattributes>> |latexmath:[p_x],
latexmath:[z] correlation. (latexmath:[R_{52}] in transport
notation.)

|`R61` |0.0 |Section <<sec.distribution.unitsdistattributes>> |latexmath:[x],
latexmath:[p_z] correlation. (latexmath:[R_{61}] in transport
notation.)

|`R62` |0.0 |Section <<sec.distribution.unitsdistattributes>> |latexmath:[p_x],
...
described in Section <<sec.distribution.gaussdisttypephotoinjector>>. Below we show an
...
[[sec.distribution.matchedgausstype]]
...
coordinates. (Section <<sec.distribution.fromfiledisttype>>).
