...
injectors, ERLs and complete XFELs.
====

[.feature-opalx]
====
_OPALX_ is a rewrite of _OPAL_ introducing C++20 and Kokkos. This endevor started in 2023 and has the goal
to enable the use of GPUs, refactoring the code and make some changes to the input structure. For example the
flavours do not exist anymore.
====
...
can be controlled using the  (see link:control#sec.control.option[Option Statement]),
...
written can be controlled with the link:control#sec.control.option[`OPTION` statement]
