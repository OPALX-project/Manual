...
image::figures/Benchmarks/Trace.png[scaledwidth=18cm]
...
image::figures/Benchmarks/TRACE_z.png[scaledwidth=12cm]
...
image::figures/Benchmarks/TRANSPORT.png[scaledwidth=18cm]
...
image::figures/Benchmarks/Input_Trace.png[scaledwidth=12cm]
...
image::figures/Benchmarks/TRACE_z_input.png[scaledwidth=12cm]
...
image::figures/Benchmarks/Trace_SBEND_edge.png[scaledwidth=18cm]
...
image::figures/Benchmarks/TRANS_SBEND_edge.png[scaledwidth=18cm]
...
image::figures/Benchmarks/T3D_Tra_SBEND_edge_env.png[scaledwidth=18cm]
...
the bending magnet. The results are reported in <<Emittance,6>> and in
<<fig_T3D_Tra_emi>>.
...
image::figures/Benchmarks/T3D_Tra_SBEND_edge_emi.png[scaledwidth=18cm]
...
magnet features of <<Bend_Trace,1>> and <<Edge_Trace,2>> have been transformed
...
image::figures/Benchmarks/SBEND_noEdge_Env.png[scaledwidth=8cm]
image::figures/Benchmarks/SBEND_noEdge_Emi.png[scaledwidth=8cm]
+
...
image::figures/Benchmarks/SBEND_Edges_Env.png[scaledwidth=8cm]
image::figures/Benchmarks/SBEND_Edges_Emi.png[scaledwidth=8cm]
+
...
++++
+
link:elements#sec.elements.RBend[RBend (_OPAL-t_)]. Instead, in TRACE 3D the field index parameter n is:
+
[latexmath]
++++
...
image::figures/Benchmarks/FI_SBEND_FMDef_Env_T2.png[scaledwidth=8cm]
image::figures/Benchmarks/FI_SBEND_FMDef_Emi_T2.png[scaledwidth=8cm]
+
...
image::figures/Benchmarks/FI_SBEND_FMTest_Env_T2.png[scaledwidth=8cm]
image::figures/Benchmarks/FI_SBEND_FMTest_Emit_T2.png[scaledwidth=8cm]
...
image::figures/Benchmarks/report-emit-dt.png[scaledwidth=18cm]

<<plot-fringe-size>> and <<plot-fringe-size-zoom>> examine the effects of
the fringe field range and the integration time step on the simulation
accuracy. <<plot-fringe-size-zoom>> is a zoom-in plot of
<<plot-fringe-size>>. We can conclude that the size of the
...
image::figures/Benchmarks/report-fringe-size.png[scaledwidth=18cm]

[#plot-fringe-size-zoom]
.Zoom in on the final emittance in <<plot-fringe-size>>
image::figures/Benchmarks/report-fringe-size-zoom.png[scaledwidth=18cm]
...
effect is turn off, <<plot-emit-csr-off>> shows that the normalized
...
image::figures/Benchmarks/emit-csr-off.png[scaledwidth=18cm]

When CSR calculations are enabled for both the bending magnet and the
following drift, <<plot-dpp-csr-on>> shows the average
latexmath:[\delta] or latexmath:[\frac{\Delta p}{p}] change along
the beam line, and <<plot-emit-csr-on>> compares the normalized
...
image::figures/Benchmarks/dpp-csr-on.png[scaledwidth=18cm]
...
horizontal emittances as indicated in <<plot-emit-csr-on>>.

[#plot-emit-csr-on]
.Transverse emittances in ELEGANT and __OPAL__
image::figures/Benchmarks/emit-csr-on.png[scaledwidth=18cm]
...
[prstab2003]. In <<plot-emit-csr-on>>, a trace-like horizontal
...
<<plot-opal-impact1>> and <<plot-opal-impact2>>. This proves to some extend
...
image::figures/Benchmarks/opal-impact-1MHz-x.png[scaledwidth=8cm]
image::figures/Benchmarks/opal-impact-1MHz-y.png[scaledwidth=8cm]

[#plot-opal-impact2]
.Longitudinal beam size and emittance in `Impact-t` and __OPAL__
image::figures/Benchmarks/opal-impact-1MHz-z.png[scaledwidth=18cm]
