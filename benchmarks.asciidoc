...
An example of TRACE 3D graphic interface is shown in Figure <<fig_trace,1>>.
...
interface <<bib.Transport_GUI>> is shown in Figure <<fig_TRANSPORT,3>>.
...
be visualized in the three phase plane as shown in Figure <<fig_Input_TRACE,4>>.
...
Figure <<fig_TRACE_z_Input,5>>. Other two cards must be added in order to use
...
phase planes for this simulation are shown in Figure <<fig_Trace_env,6>>.
...
In case of the modified sigma-matrix in Figure <<fig_TRACE_z_Input,5>>, the
...
++++
+
Section link:elements#sec.elements.RBend[RBend (_OPAL-t_)]. Instead, in TRACE 3D the field index parameter n is:
+
[latexmath]
++++
...
emittance, as shown in Figure <<fig_SBEND_FI,12>> using:
+
....
// Bending magnet configuration:
K1=1.5
...
shown in Figure <<fig_SBEND_FI_test,13>>, is achieved using a test field map in
...
TRACE 3D emittance see Figure <<fig_SBEND_FI_test,13>>.
