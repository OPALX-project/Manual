...
<<bib_TraceMan>>. It provides an instantaneous beam profile diagram and
...
conversion can be displayed using the _command W_ (see <<bib_TraceMan>> page
42).
...
<<bib_transport>>. The TRANSPORT version for Windows provides a graphic
beam profile diagram, as well as a sigma matrix description of the
simulated beam and line <<bib_Transport_GUI>>. Differently from TRACE 3D, the
...
on TRANSPORT documentation <<bib_transport>> for more details).
...
[#fig:trace_z]
.Figure 2: Sigma-matrix structure in TRACE 3D <<bib_TraceMan>>
...
interface <<bib_Transport_GUI>> is shown in Figure <<fig:TRANSPORT,3>>.

[#fig:TRANSPORT]
.Figure 3: GUI TRANSPORT graphic interface <<bib_Tran_ex>>. The continuous lines describe the beam envelope in the vertical plane (above) and horizontal plane (below). The dashed line displays the dispersion. The elements in the beam line are drawn as blue and red rectangles
...
between _OPAL_ and ELEGANT <<bib_elegant>> for example, one needs to
...
* [[[bib_TraceMan]]] K. Crandall and D. Rusthoi, _Documentation for trace: an interactive beam-transport code_, tech. rep. (Los Alamos National Laboratory).
* [[[bib_transport]]] K. L. Brown et al., _TRANSPORT - A Computer Program for Designing Charged Particle Beam Transport Systems_, tech. rep. CERN 73-16, revised as CERN 80-4 (CERN, 1980).
* [[[bib_TransportGUI]]] U. Rohrer, _PSI graphic transport framework based on a cern-slac-fermilab version by K.L. Brown et al_, tech. rep. (PSI).
* [[[bib_Tran_ex]]] U. Rohrer, http://aea.web.psi.ch/Urs_Rohrer/MyWeb/moregifs/gantryb.gif.
* [[[bib_elegant]]] M. Borland, https://www1.aps.anl.gov/icms_files/lsnotes/files/APS_1418218.pdf[_Elegant: a flexible SDDS-compliant code for accelerator simulation_], tech. rep. LS-287 (Advanced Photon Source, Sept. 2000).
