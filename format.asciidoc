...
identifier see Section <<sec.format.label>> and gives a name to the stored command.
2.  The `keyword` identifies the action desired. It must be an
identifier see Section <<sec.format.label>>.
...
see Section <<sec.format.label>>.
* The `attribute-value` gives it a value see Section <<sec.format.attribute>>. When
...
+
Each attribute has a fixed attribute type see Section <<sec.format.attribute>>. +
...
expression see Section <<sec.format.astring>>.
...
If the attribute is an array see Section <<sec.format.anarray>>, one of its
...
* String see Section <<sec.format.astring>>,
* Logical see Section <<sec.format.alogical>>,
* Real expression see Section <<sec.format.areal>>,
* Deferred expression see Section <<sec.format.adefer>>,
* Place see Section <<sec.format.aplace>>,
* Range see Section <<sec.format.arange>>,
* Constraint see Section <<sec.format.aconstraint>>,
* Variable Reference see Section <<sec.format.areference>>
* Regular expression see Section <<sec.format.wildcard>>
* Array see Section <<sec.format.anarray>> of
** Logical see Section <<sec.format.logarray>>,
** Real see Section <<sec.format.realarray>>,
** String see Section <<sec.format.strarray>>,
** Token lists see Section <<sec.format.tokarray>>,
...
* Operand see Section <<sec.format.operand>>,
* Random generators see Section <<sec.format.adefer>>.
...
String values can occur in string arrays see Section <<sec.format.anarray>>.
...
logical expression can occur in logical arrays see Section <<sec.format.logarray>>.
...
see Section <<sec.format.realarray>>.

A real expression is built from operators see Table <<operator,4>> and
operands see Section <<sec.format.operand>>:
...
functions see Table <<realfun,5>> acting on operands see Section <<sec.format.operand>>.
...
see Table <<constant,7>>. Their names must not be used for
...
see Section <<sec.format.avariable>>, the second format refers to a named `attribute`
...
see Section <<sec.format.label>>. Since identifiers are converted to lower case,
...
see Section <<sec.format.attribute>>. Normally an array is entered as a list in
braces:
...
permissible values String values see Section <<sec.format.astring>>.
